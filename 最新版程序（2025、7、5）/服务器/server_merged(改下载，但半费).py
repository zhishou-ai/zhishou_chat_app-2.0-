                ...
                if isinstance(message, bytes):
                    ...
                    if file_info:
                        ...
                        if receiver_type == "group":
                            save_dir = os.path.join(base_dir, "group", f"group_{receiver_id}")
                        else:  # private
                            # 获取发送者ID
                            sender_id = None
                            ...
                            # 检查sender_id和receiver_id是否存在
                            if not sender_id:
                                logger.error("发送者ID不存在")
                                await websocket.send(json.dumps({"error": "发送者未登录或连接已断开"}))
                                return
                            if not receiver_id:
                                logger.error("接收者ID不存在")
                                await websocket.send(json.dumps({"error": "接收者ID不能为空"}))
                                return
                            
                            # 对两个ID排序以确保文件夹名称唯一
                            sorted_ids = sorted([str(sender_id), str(receiver_id)])
                            folder_name = f"private_{sorted_ids[0]}_{sorted_ids[1]}"
                            save_dir = os.path.join(base_dir, "private", folder_name)
                    ...
                    try:
                        ...
                        if data.get("type") == "file_info":
                            ...
                            if action == "login":
                                ...
                                if receiver_type == "group":
                                    file_path = os.path.join(base_dir, "received_files", "group", f"group_{receiver_id}", filename)
                                else:  # private
                                    if sender_id and receiver_id:
                                        sorted_ids = sorted([str(sender_id), str(receiver_id)])
                                        folder_name = f"private_{sorted_ids[0]}_{sorted_ids[1]}"
                                        file_path = os.path.join(base_dir, "received_files", "private", folder_name, filename)
                                    else:
                                        file_path = os.path.join(base_dir, "received_files", filename)  # 回退到旧路径
                                print(file_path)
