...
function loadContactLists(type) {
    ...
    let url;
    if (type === 'private') {
        url = 'http://192.168.3.106:80/history/users';
    } else {
        const userId = localStorage.getItem('currentUserId');
        url = `http://192.168.3.106:80/history/groups?user_id=${userId}`;
    }
...
function loadChatMessages() {
    ...
    const xhr = new XMLHttpRequest();
    xhr.open('POST', 'http://192.168.3.106:80/history/messages', true);
...
function updateUserInfo() {
    const xhr = new XMLHttpRequest();
    xhr.open('GET', `http://192.168.3.106:80/history/user_info?id=${localStorage.getItem('currentUserId')}`, true);
