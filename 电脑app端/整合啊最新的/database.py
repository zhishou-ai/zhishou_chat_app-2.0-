...
class DatabaseManager:
        ...
        try:
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS users (
                    ...
                    permission TEXT NOT NULL DEFAULT 'student'  -- 修改字段名
                )
            """)
    ...
    def save_user(self, user_id, username, password, permission='student'):
        try:
            conn = sqlite3.connect(self.db_name)
            cursor = conn.cursor()
            ...
            if existing_user:
                # 更新现有用户（包含权限）
                cursor.execute(
                    "UPDATE users SET username = ?, permission = ? WHERE id = ?",
                    (username, permission, user_id)  # 添加权限字段
                )
            else:
                # 插入新用户（包含权限）
                cursor.execute(
                    "INSERT INTO users (id, username, password, permission) VALUES (?, ?, ?, ?)",
                    (user_id, username, password, permission)  # 添加权限字段
                )
    ...
    def save_message(self, user_id, sender_id, sender_name, receiver_id, receiver_type, content, timestamp, is_sent=0, is_file=False):
        """保存消息到数据库"""
        try:
            conn = sqlite3.connect(self.db_name)
            cursor = conn.cursor()
            cursor.execute(
                ...
                (user_id, sender_id, sender_name, receiver_id, receiver_type, content, timestamp, is_sent, is_file) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (user_id, sender_id, sender_name, receiver_id, receiver_type, content, timestamp, is_sent, 1 if is_file else 0)
            )
            conn.commit()
            return True
        except sqlite3.Error as e:
