...
class WebSocketClient(QThread, QObject):
    ...
    def send_file(self, file_bytes, filename, receiver_id, receiver_type):
        """发送文件（如语音、图片等）到服务端"""
        if self.loop and self.running and not self.loop.is_closed():
            asyncio.run_coroutine_threadsafe(
                self._send_file_async(file_bytes, filename, receiver_id, receiver_type), self.loop
            )

    async def _send_file_async(self, file_bytes, filename, receiver_id, receiver_type):
        try:
            if self.websocket is None or self.websocket.closed:
                print("WebSocket 未连接，尝试重新连接...")
                await self.connect()
            if self.websocket and not self.websocket.closed:
                ...
                # 2. 再发二进制内容
                await self.websocket.send(file_bytes)
                print(f"已发送文件: {filename}")
            else:
                print("无法发送文件：WebSocket 连接不可用")
        except Exception as e:
            print(f"发送文件失败: {e}")
            self._connected = False
            self.connection_changed.emit(False)